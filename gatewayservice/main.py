...
This file implements a HTTPS gatewayservice, it is deployed in GAE.
...
def process_msg():
  ...
  try:
    ...
    query = ('INSERT INTO EnergyDisaggregation.ActivePower (time, device_id, power) '
             'VALUES (timestamp("{}"), "{}", {});'.format(time_stamp[-1], device_id, active_power[-1]))
    ...
    logging.info('4. Results recorded: {}'.format(rows_to_insert))
    if errors:
